...
public class CreateFileLogic {
	 ...
	 * tmpFile
	 */
//		public Path createFile(Path dir) throws IOException {
...
//現未使用
//	public Path createTmpDirectry(Path path) throws IOException {
//		Path createdDirectry=null;
//		try {
//			createdDirectry=Files.createTempDirectory(path, null);//すでに存在していて作成できなかった場合でも例外はスローされません。<->createDirectory
//
//		} catch (UnsupportedOperationException e1) {
//			e1.printStackTrace();
//			throw new UnsupportedOperationException("ディレクトリの作成時に原子的に設定できない属性が配列に含まれる");
...
//	}
//✔︎Path.resolve()　検証
//	dir=glogic.getTmpPath(ServletContext context);
	public Path createUploadFile(Path dir,String filename) throws IOException {
//		Path createdfile = Paths.get(path.toString(),filename);//null
